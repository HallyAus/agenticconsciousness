...
export default function ToolGate({ toolId, children }: { toolId: string; children: ReactNode }) {
  ...
  if (state === 'loading') {
    return (
      <div className="flex items-center justify-center py-20">
        <div className="font-mono text-[0.8rem] max-sm:text-xs tracking-[2px] uppercase text-text-dim animate-pulse">
          Loading tool...
        </div>
      </div>
    );
        ...
        <div className="max-w-[400px] mx-auto text-center">
          ...
          <div className="flex gap-0 max-sm:flex-col max-sm:gap-3">
            ...
            <button
              ...
              disabled={submitting}
              className="bg-ac-red text-white font-display text-[0.8rem] max-sm:text-xs font-black tracking-[2px] uppercase px-6 py-3 transition-all duration-200 hover:bg-white hover:text-ac-black cursor-pointer border-none disabled:opacity-50"
            >
              {submitting ? 'Sending...' : 'Continue'}
            </button>
          </div>
          {emailError && <p className="text-ac-red text-[0.8rem] max-sm:text-xs font-mono mt-2">{emailError}</p>}
          <p className="text-text-ghost text-[0.75rem] max-sm:text-xs font-mono mt-4">
    ...
    return (
      <div className="border-2 border-ac-red p-8 max-sm:p-5 bg-ac-card text-center">
        ...
        <div className="mt-6 flex items-center justify-center gap-2">
          ...
          <span className="font-mono text-[0.75rem] max-sm:text-xs tracking-[2px] uppercase text-text-dim">
  ...
  if (state === 'capped') {
    return (
      <div className="border-2 border-ac-red p-8 max-sm:p-5 bg-ac-card text-center">
        ...
        <a
          href="/#contact"
          className="inline-block bg-ac-red text-white font-display text-[0.8rem] max-sm:text-xs font-black tracking-[2px] uppercase py-3 px-8 no-underline transition-all duration-200 hover:bg-white hover:text-ac-black"
        >
          Book a consultation &rarr;
        </a>
        <p className="text-text-ghost text-[0.75rem] max-sm:text-xs font-mono mt-6">
  ...
  return (
    <ToolGateContext.Provider value={{ onToolUse, remainingUses: usage.remainingUses, tier: usage.tier }}>
      {showBanner && (
        <div
          className="py-2 px-4 text-center font-mono text-[0.75rem] max-sm:text-xs tracking-[2px] uppercase border-b-2"
          style={{
